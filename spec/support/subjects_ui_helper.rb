module SubjectsUiHelper
  ...
  def image_editor_loaded! image, expected_linkables=nil
    within("pt-image-editor .image-form") do
      expect(page).to have_css("span.image-id",:text=>image.id,:visible=>false)
    ...
    if expected_linkables && logged_in?
      expect(page).to have_css(".linkable-things select option",
          :count=>expected_linkables)
    end
  end

  def visit_image image
    unless page.has_css?("pt-image-editor .image-form span.image-id",
  ...
  def visit_thing thing
    unless page.has_css?("pt-thing-editor .thing-form span.thing-id",
  ...
  def thing_editor_loaded! thing
    ...
    within("pt-thing-editor .thing-form") do
      expect(page).to have_css("span.thing-id",:text=>thing.id,
                                               :visible=>false)
      expect(page).to have_css("ul.thing-images li span.image-id",
                              :visible=>false,

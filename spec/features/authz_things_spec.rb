...
RSpec.feature "AuthzThings", type: :feature, js:true do
  ...
  def visit_thing thing
    ...
    within("pt-thing-editor") do
      expect(page).to have_css(".thing-form span.thing-id",
  ...
  def visit_things
    ...
    within("pt-thing-selector", :wait=>5) do
      if logged_in?
  ...
  shared_examples "can list things" do
    it "lists things" do
      visit_things
      within("pt-thing-selector .thing-list") do
        things.each do |t|
          expect(page).to have_css("li a",:text=>t.name)
          expect(page).to have_css(".thing-id",:text=>t.id,:visible=>false)
          expect(page).to have_no_css(".thing-id") #should be hidden
        end
      end
    end
  end
  ...
  shared_examples "originator has invalid thing" do
    it "cannot edit description for invalid thing" do
      within("pt-thing-editor .thing-form") do
        ...
        expect(page).to have_field("thing-description", :visible=>false)
  ...
  shared_examples "can create valid thing" do
    it "creates thing" do
      within ("pt-thing-editor .thing-form") do
        ...
        fill_in("thing-description", :with=>thing_props[:description])
  ...
  shared_examples "displays thing" do
    it "can display specific thing" do
      visit_thing thing
      within("pt-thing-editor .thing-form") do
        expect(page).to have_css(".thing-id", :text=>thing.id, :visible=>false)
  ...
  shared_examples "can clear thing" do
    it "clears thing" do
      within("pt-thing-editor .thing-form") do
        ...
        expect(page).to have_no_css(".thing-id", :text=>thing.id, :visible=>false)
        expect(page).to have_field("thing-name", :with=>"")
        expect(page).to have_field("thing-description", :visible=>false, :with=>"")
  ...
  shared_examples "cannot see details" do
    it "hides details" do
      within("pt-thing-editor .thing-form") do
        expect(page).to have_field("thing-name", :with=>thing.name)
        expect(page).to have_field("thing-description", :with=>thing.description)
        expect(page).to have_field("thing-notes",:visible=>false)
      end
    end
  end
  ...
  shared_examples "cannot update to invalid thing" do
    ...
    it "cannot update with invalid description" do
      update_text_field "thing-description"
    end
  ...
  context "things posted" do
    before(:each) do
      ...
      visit "#{ui_path}/#/things/"
      logout
      expect(page).to have_css("pt-thing-selector")
    end
    after(:each) { logout }

    def select_thing
      within("pt-thing-selector .thing-list") do
        find("span.thing-id",:text=>thing.id, :visible=>false).find(:xpath,"..").click
      end
      within("pt-thing-editor .thing-form") do
        expect(page).to have_css("span.thing-id",:text=>thing.id, :visible=>false)
      end
    end
    ...
    context "user logs out" do
      it "displays last selected thing as non-member" do
        ...
        within("pt-thing-editor .thing-form") do
          expect(page).to have_field("thing-name", :with=>thing.name)
          expect(page).to have_field("thing-description", :visible=>true,
                                                   :readonly=>false)

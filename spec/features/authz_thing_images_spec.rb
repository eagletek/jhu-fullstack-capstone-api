...
RSpec.feature "AuthzThingImages", type: :feature, :js=>true do
  ...
  shared_examples "can get links" do
    context "from images" do
      ...
      it "can view linked things for image" do
        within("pt-image-editor") do
          ...
          within("div.image-things") do
            ...
            expect(page).to have_css("li span.thing-id",
                                     :text=>linked_thing.id,
                                     :visible=>false)
            expect(page).to have_no_css("li span.thing-id") #should be hidden
          end
        end
      ...
      it "can navigate from image to thing" do
        expect(page).to have_css("pt-image-editor")
        link_selector_args=["pt-image-editor ul.image-things span.thing-id",
        ...
        using_wait_time 5 do
          ...
          expect(page).to have_no_css("pt-image-editor")
    ...
    context "from things" do
      ...
      it "can view linked images for thing" do
        expect(page).to have_css("pt-thing-editor")
        within("pt-thing-editor .thing-form ul.thing-images",:wait=>5) do
          expect(page).to have_css("li span.image-id",
      ...
      it "can navigate from thing to image" do
        ...
        #wait for the link to show up and then click
        within("pt-thing-editor .thing-form ul.thing-images") do
          node=find("li span.image-id",:text=>linked_image.id,
                                  :visible=>false,
                                  :wait=>5).find(:xpath,"../label").click
        end

        #wait for page to react to link and switch away
        expect(page).to have_no_css("pt-thing-editor")
  ...
  shared_examples "can create links" do
    ...
    it "can get linkable things for image" do
      ...
      within("pt-image-editor .image-form > .linkable-things") do
        expect(page).to have_css(".linkable-things select option", :count=>2, :wait=>5)
        (1..2).each do |i|
          expect(page).to have_css(".linkable-things select option", :text=>things[i].name)
          expect(page).to have_css(".linkable-things select option[value='#{things[i].id}']")
        end
        # verify page does not contain option to already linked things
        expect(page).to have_no_css(".linkable-things select option[value='#{linked_thing.id}']")
      end
    ...
    it "can create link image to things" do
      within("pt-image-editor .image-form") do
        ...
        #click on option
        find(".linkable-things select option", :text=>things[1].name).select_option
        ...
        expect(page).to have_css("ul.image-things li span.thing-id",
    ...
    it "removes thing from linkables when linked" do
      within("pt-image-editor .image-form") do
        ...
        expect(page).to have_no_css(".linkable-things select option", :text=>things[1].name)
    ...
    it "removes link button when no linkables" do
      within("pt-image-editor .image-form") do
        ...
        all(".linkable-things select option").each do |option|
  ...
  shared_examples "can edit link" do
    ...
    it "can view priority of image" do
      within("pt-thing-editor .thing-form .thing-images ul") do
    ...
    it "can adjust priority of image" do
      within("pt-thing-editor .thing-form") do
